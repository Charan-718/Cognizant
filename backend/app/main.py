...
class MemorySearchRequest(BaseModel):
    """Request body for vector memory search."""
    query: str
...
@app.get(f"{API_PREFIX}/metrics/{{request_id}}")
...
@app.websocket(f"{API_PREFIX}/stream/{{request_id}}")
...
@app.get(f"{API_PREFIX}/status/{{request_id}}")
...
@app.post(f"{API_PREFIX}/ideas/{{workflow_id}}/search")
async def search_similar_ideas(workflow_id: str, request: MemorySearchRequest):
    """
    ...
    try:
        ...
        memory = MemoryStore()
        results = memory.search_similar_ideas(request.query, workflow_id, k=5)
...
@app.get(f"{API_PREFIX}/evolution/{{workflow_id}}")
...
@app.get(f"{API_PREFIX}/health/dependencies")
